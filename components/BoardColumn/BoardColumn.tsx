...
export default function BoardColumn({ boardData, cardTaskModal, tasks, setCardTaskModal, setSelectedColumnId }: PropsType) {
   ...
   // - DRAG AND DROP -
   const { attributes, listeners, setNodeRef, transform, transition, } = useSortable({ id: boardData._id, data: { type: "column", column: boardData }, });
   const { setNodeRef: setDropRef, isOver } = useDroppable({ id: boardData._id, data: { type: "column", column: boardData, }, });
            ...
            <div className="sticky top-0 z-10 rounded-t-2xl border-b bg-white p-4">
               <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                     ...
                     <h2 className="font-semibold text-gray-800"> {boardData.title} </h2>
                  ...
                  <div onClick={ColMenuHandler} className={`hover:text-gray-600 transition-all duration-200 flex flex-col relative px-2 items-center `} >
                     <Button variant="ghost" size="icon" className={"opacity-0 transition-opacity group-hover:opacity-100" + (OpenColMenu ? " bg-gray-100 rounded-sm opacity-100" : "")}> <MoreHorizontal className="size-5" /> </Button>
                     {
                        <div className={"absolute right-0 top-full w-48 rounded-lg border bg-white shadow-lg" + (OpenColMenu ? " opacity-100 visible" : " invisible opacity-0")}>
                           <button onClick={() => setEditColData(true)} className="flex w-full items-center gap-2 px-3 py-2 hover:bg-gray-100"><Pencil className="h-4 w-4" />Edit Board</button><hr />
                           <button onClick={() => setDeleteColModal(true)} className="flex w-full items-center gap-2 px-3 py-2 text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4" />Delete Board</button>
                        </div>
                     }
                  </div>
               </div>
